...
const UserSchema = new mongoose.Schema({
  ...
  countryCode: {
    type: String,
    default: '+52', // Default to Mexico's country code
    trim: true,
    validate: {
      validator: function(v) {
        // Validate country code format (e.g., +1, +52, +44)
        return /^\+\d{1,4}$/.test(v);
      },
      message: 'Formato de código de país inválido (ejemplo: +52)'
    }
  },
  phone: {
    type: String,
    default: '',
    trim: true,
    // ✅ ACTUALIZADO: Validación de teléfono que funciona con countryCode
    validate: {
      validator: function(v) {
        // Si está vacío, es válido (opcional)
        if (!v || v.length === 0) return true;
        // Validar formato básico de teléfono (solo dígitos, sin espacios ni caracteres especiales)
        return /^\d{7,15}$/.test(v);
      },
      message: 'Formato de teléfono inválido (solo números, 7-15 dígitos)'
    },
    // ✅ AGREGADO: Getter para obtener el teléfono completo con código de país
    get: function() {
...
UserSchema.methods.getPublicProfile = function() {
  try {
    ...
    if (userObject.averageRating !== undefined) {
      userObject.averageRating = Number(userObject.averageRating) || 0;
    }
    
    if (userObject.totalRatings !== undefined) {
      userObject.totalRatings = Number(userObject.totalRatings) || 0;
    }
    
    // Condicional para campos sensibles según preferencias
    if (!this.preferences.showPhone) {
      delete userObject.phone;
      delete userObject.countryCode;
    } else if (userObject.phone) {
      // Asegurar que el teléfono se muestre con el código de país
      userObject.phone = this.countryCode + userObject.phone;
    }
    ...
    if (!this.preferences.publicProfile) {
      ...
      return {
        ...
        totalRatings: userObject.totalRatings || 0,
...
UserSchema.virtual('ratingDisplay').get(function() {
  const total = Number(this.totalRatings) || 0;
  const avg = Number(this.averageRating) || 0;
  
  if (total === 0) return 'Sin calificar';
  
  try {
    return `${avg.toFixed(1)} (${total} reseña${total !== 1 ? 's' : ''})`;
  } catch (error) {
    console.error('Error formateando ratingDisplay:', { 
...
UserSchema.virtual('memberSince').get(function() {
  try {
    const now = new Date();
    const joined = this.date ? new Date(this.date) : now;
    
    // Validar fecha
    if (isNaN(joined.getTime())) {
      console.warn('Fecha de registro inválida:', this.date);
    ...
    console.error('Error calculando memberSince:', error);
    return 'Recién registrado';
  }
});
...
UserSchema.set('toJSON', { 
  ...
  transform: function(doc, ret) {
    try {
      // Asegurar que los campos numéricos sean números
      if (ret.averageRating !== undefined) {
        ret.averageRating = Number(ret.averageRating) || 0;
      }
      
      if (ret.totalRatings !== undefined) {
        ret.totalRatings = Number(ret.totalRatings) || 0;
      }
      ...
      // Formatear teléfono con código de país si está presente
      if (ret.phone && !ret.phone.startsWith('+') && ret.countryCode) {
        ret.phone = ret.countryCode + ret.phone;
      }
      
      return ret;
    } catch (error) {
      console.error('Error en toJSON transform:', error);
      // Devolver un objeto seguro en caso de error
      return {
        _id: ret._id,
        name: ret.name || 'Usuario',
        avatar: ret.avatar,
